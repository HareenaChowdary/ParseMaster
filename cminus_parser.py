...
# Define a rule for floating-point constants
def t_FLOATCON(t):
    r'[0-9]+\.[0-9]+'
    t.value = float(t.value)
    return t

# Define a rule for string constants
def t_STRING(t):
    r'\'[a-zA-Z0-9\']+\''
    return t
...
def p_IOStatement(p):
    ...
    if p[1] == 'read':
        ...
        # Check if it's a tuple, indicating it's an Expr
...
def test_lexer(data):
    ...
    while True:
        ...
        if not token:
            break  # No more input
        lex_output.append((token.type, token.value))
    print("### Lex Output:")
    for item in lex_output:
        print(item)
# Test the parser with input from a file
file_path = r'/Users/hareena/Documents/CS5950/pa4-parser-Hareena-Polavaram/cminusinput3.c'
